 ...
 */
@WebServlet("/manage/admin_donewsbulletinselect")
public class DoNewsBulletinSelect extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int arr[] = ELECTRONIC_NEWS_BULLETINDao.totalPage(count,keyword);
		//selectall返回所有用户对象 使用列表接收 也就是用户实体类型
		//分页跟查询都在selectall执行 查询在全选择完进行查询 where 字段 %keywoerd%
		ArrayList<ELECTRONIC_NEWS_BULLETIN>  list = ELECTRONIC_NEWS_BULLETINDao.selectAll(cpage,count,keyword);
		//放到请求对象域里
		...
		//普通重定向戴不进去 
		request.getRequestDispatcher("admin_news_bulletin.jsp").forward(request, response);
	}
