 ...
 */
@WebServlet("/manage/admin_donewsbulletindel")
public class DoNewsBulletinDel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int count = ELECTRONIC_NEWS_BULLETINDao.del(id);
		//成功失败重定向到哪里
				if(count > 0) {
					...
					out.write("location.href='manage/admin_donewsbulletinselect?cp="+request.getParameter("cpage")+"'");
					out.write("</script>");
				}
	}
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<ids.length;i++) {
			int count = ELECTRONIC_NEWS_BULLETINDao.del(ids[i]);
		}
		//成功重定向到哪里
				
		response.sendRedirect("/electronic/manage/admin_donewsbulletinselect" );
	}
	}
